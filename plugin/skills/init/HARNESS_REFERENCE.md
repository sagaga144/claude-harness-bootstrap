...
| `PreModelSwitch` (optional) | cost guard | If Step 1's guided cost answer was "keep cost low," block a switch to a pricier model the session didn't explicitly ask for — ties the cost-priority choice to an actual enforcement point, not just an agent-file default. Payload carries `to_model`/`from_model` (not `model`, which is `SessionStart`-only) — check `to_model` for the target. **Block the same way every other `command` hook in this harness does — exit code 2, with the reason on stderr** — this event supports that plain contract exactly like `PreToolUse` does, there's no need to reach for structured `permissionDecision` JSON output just because this event's own docs happen to describe it via that field; consistency with the rest of the harness's hooks matters more than which valid mechanism gets used |
...
thin project-specific wrapper around it, instead. A thin-wrapper command can still pin the
model this table calls for even though there's no agent-file frontmatter to put it in: the
model resolves in order (1) the per-invocation `model` parameter on the Agent-tool call,
(2) the agent's own frontmatter, (3) an env var, (4) the main conversation's model — so
have the wrapper's instructions tell Claude to pass that model explicitly on the
invocation, not just describe a tier "expected" in CLAUDE.md's table with nothing actually
enforcing it.
...
Apply the cap even to a stakes-justified reviewer — don't quietly except it. But when that
reviewer is gating something whose failure mode is unusually severe even by this table's
own standard (not just "a missed authz hole," but something closer to irreversible —
destroying real infrastructure, an unrecoverable data-loss path), name that specific
tension plainly in the final report rather than applying the cap silently: "this reviewer
is capped at Sonnet under your cost answer, even though it's the one check standing
between a change and [the real consequence] — say so if you'd rather it ran on Opus."
That's surfacing a real tradeoff for the user to react to, not overriding their answer.
...
| Correctness reviewer | one reviewer for the primary language's own failure modes (type errors, unhandled `Result`/`Option`, panics, goroutine races/deadlocks or other unsynchronized-shared-state bugs under concurrency, etc. — whatever "compiles but wrong" looks like in this ecosystem, including bugs that only show up at runtime under load, not just at compile time). For the two-toolchain-one-product case above, one reviewer covering both is the same default as the implementer row — it just needs to actually shell out to *both* toolchains' checks, not silently cover only one. If the layer's language/setup has no mechanical "compiles but wrong" ground truth at all (no compiler, no build step — a no-framework vanilla-JS client), the reviewer's method shifts to inspection-based checks (protocol/contract mismatches against what the other layer actually sends, runtime error patterns) rather than a tool's exit code — a different method, not a reason to skip the layer entirely. Unlike implementers, reviewers are good candidates for merging across genuinely separate layers into one agent with clearly separated sections (per "default to fewer, broader agents" above) precisely because review is read-only with no conflicting behavior mode — a single reviewer covering an infra layer and an application layer, say, is a reasonable default, not a stretch; split only when a layer's review genuinely needs isolated context. **In some domains "correctness" and the trust-boundary row below aren't separable concerns at all — merge the correctness reviewer and `security-reviewer` into one agent rather than building both, when that's true.** Smart-contract review is the clearest case: a reentrancy bug is simultaneously a logic error and the exploit itself, not two different findings a compiler-shaped check and a trust-boundary check would surface separately the way they would in a typical web app (a null-pointer crash and a missing auth check are unrelated bugs there). **A second, differently-shaped case: hard real-time / deadline-bound execution — an audio callback, a control loop, an ISR, anything with a scheduler-enforced deadline where a single violation causes a glitch, not a crash or a wrong number.** The vocabulary is its own (real-time-safe, lock-free, no allocation/no locking/no blocking on the deadline-bound path) and it doesn't fit the *Perf-sensitive* row below despite that row's "real-time budgets" phrase sounding closest — perf-sensitive is a gradient (optimize until fast enough), this is binary (violated or not, every single call). Fold it into the correctness reviewer the same way as the smart-contract case, not into `performance-reviewer`. Notice when the domain's actual failure modes don't split cleanly and merge rather than forcing the standard two-reviewer shape | Haiku if mechanically checkable (e.g. `tsc --noEmit`, `mypy`), else Sonnet |
...
| Has auth, secrets, direct user input, **or any other trust boundary between untrusted and privileged code — the untrusted side doesn't need to be a distinguishable "role" (logged-in vs. guest) to count; an anonymous, symmetric caller is still untrusted** — a desktop app's webview-to-native IPC/capability boundary (Tauri/Electron), a browser extension's content-script-to-background boundary, a plugin sandbox, a multiplayer/game server that must not trust client-reported state (position, score, actions — the anti-cheat concern), a smart contract where *any address on the chain* can call any public function with any arguments any number of times — no "guest" to distinguish from a "user," everyone is symmetric until an explicit check says otherwise. Naming this row's breadth explicitly matters: skip it too easily and a project loses its one load-bearing reviewer entirely under essentials rather than deferring something safe to defer | `security-reviewer` | Opus |
| Perf-sensitive — needs to be *fast enough*, a gradient you optimize until it's acceptable (heavy compute, large data, mobile battery) — **not** the hard-real-time/deadline-bound case (a binary, every-call constraint; see the correctness-reviewer row above) | `performance-reviewer` | Sonnet |
...
of **six** different things depending on the profile — pick (or combine) accordingly,
...
- **Infrastructure change** (a Terraform/Pulumi/CloudFormation apply, a `kubectl apply`, an Ansible playbook run against a real environment): don't force this into "scheduled job" or "live service" — it's neither a recurring unwatched process nor a build reaching a "ready" state, it's a deliberate, singular, human-gated mutation of a real system, and it deserves its own discipline. The core risk is the same shape as scheduled-job's ("exit code 0 ≠ it did what you meant") but the failure mode is different: a plan/apply can succeed exactly as written while still being the *wrong* diff — one nobody actually reviewed against the intended change. Confirm the plan output was reviewed against the stated goal before apply, not just that apply returned 0; if the harness has no live credentials to check the target system's actual resulting state (the common case — a coding session shouldn't hold prod AWS/cloud credentials by default), say so plainly as a real gap rather than implying a health check happened that didn't.
- **Irreversible/immutable deployment** (a deployed smart contract with no upgrade proxy, firmware flashed to hardware with no OTA update path, any artifact where there is genuinely no post-release patch — not "patching is expensive," but "there is no mechanism to patch at all"): this is not a combination of the flavors above, it needs its own discipline because every other flavor's health check assumes a meaningful *after* — a live service you can smoke-test, a published artifact you can yank or point users to a patched version of, an infra change you could re-apply. Here there isn't one. Front-load the entire health check to *before* the irreversible action: the equivalent of "confirm it's ready" has to mean the specialist reviewer signed off on the *exact* artifact about to ship (not an earlier draft), every configuration/constructor argument was reviewed, and a human explicitly said go — because nothing catches a miss after the fact. Build a pre-flight check (a `/predeploy-check`-style command), not a post-deploy one.
